...
function inferLineupStatus(entry, bundle) {
  ...
  if (scheduleDisposition === 'overseas') {
    if (rawStatus.includes('returning') || rawStatus.includes('planned') || rawStatus.includes('production') || inProduction) {
      return { variant: 'returning', text: 'Returning, but no US release date is confirmed yet.' };
    }
    return { variant: 'returning', text: 'Aired overseas; US release date not confirmed.' };
  }
  ...
  if (rawStatus.includes('returning') || rawStatus.includes('planned') || rawStatus.includes('production') || inProduction) {
    return { variant: 'returning', text: 'New season expected, but no US release date is announced yet.' };
  }
  ...
  return bundle
    ? { variant: '', text: 'No known US release date right now.' }
    ...
    <div class="lineup-card__full">
      <div class="lineup-card__full-top">
        ...
        <div class="lineup-card__summary">
          ...
          <div class="lineup-card__stats">
            <div><span>Main channel</span><strong>${escapeHtml(bundle.mainChannel || 'Unknown')}</strong></div>
            <div><span>Service</span><strong>${escapeHtml(bundle.streaming || 'Unknown')}</strong></div>
      ...
      <div class="lineup-card__table-wrap">
        <table class="lineup-card__table">
          <thead><tr><th>Season</th><th>Episodes</th><th>Release</th><th>US next</th></tr></thead>
...
function buildWatchedForRemote(show) {
  ...
  watched.__tvtMeta = { archived: Boolean(show.archived), archivedAt: show.archivedAt || '' };
  return watched;
}
