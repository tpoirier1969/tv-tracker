...
function getUserScopedShows() {
  ...
  if (state.upcomingFilter === 'archived') return scoped.filter((show) => Boolean(show.archived));
...
function buildInlineDetailHtml(entry, bundle) {
  ...
  const meta = getShowMeta(entry);
  const seasonRows = (bundle.seasons || []).slice(0, 8).map((season) => {
  ...
  const episodeSeasonBlocks = !entry.archived
    ? (bundle.seasons || []).map((season) => {
        ...
        return `<details class="episode-watch-season"><summary><span>Season ${escapeHtml(String(season.season ?? '—'))}</span><span class="episode-watch-season__count">${watchedCount}/${episodeList.length} watched</span></summary><div class="episode-watch-list">${episodeItems}</div></details>`;
      }).filter(Boolean).join('')
    : '';

  const episodeWatchHtml = episodeSeasonBlocks
    ? `<section class="episode-watch-panel"><h4>Episode watch tracker</h4><p class="episode-watch-panel__hint">Check off episodes you have watched. This syncs with the show record.</p>${episodeSeasonBlocks}</section>`
    : (!entry.archived ? '<section class="episode-watch-panel"><h4>Episode watch tracker</h4><p class="episode-watch-panel__hint">Episode-level data is not loaded for this series yet.</p></section>' : '');

  return `
    <div class="lineup-card__full">
      <div class="lineup-card__full-top">
        ${poster ? `<div class="lineup-card__poster"><img src="${escapeAttr(poster)}" alt="${escapeAttr(bundle.show?.name || entry.name || '')} poster"></div>` : ''}
        <div class="lineup-card__summary">
          <p>${escapeHtml(summary)}</p>
          <div class="lineup-card__stats">
            <div><span>Main channel</span><strong>${escapeHtml(inferProviderFamily(bundle.mainChannel || 'Unknown') || 'Unknown')}</strong></div>
            <div><span>Service</span><strong>${escapeHtml(compactProviderList(bundle.streaming || 'Unknown') || 'Unknown')}</strong></div>
            <div><span>Status</span><strong>${escapeHtml(inferLineupStatus(entry, bundle).text)}</strong></div>
            <div><span>Assigned</span><strong>${escapeHtml(assignedText)}</strong></div>
          </div>
        </div>
      </div>
      ...
      <div class="lineup-card__status-tools">
        <label>
          ...
          <select class="lineup-card__status-override">
            ...
            <option value="returning" ${meta.statusOverride === 'returning' ? 'selected' : ''}>Returning</option>
            <option value="in-development" ${meta.statusOverride === 'in-development' ? 'selected' : ''}>Revival in development</option>
            <option value="overseas-only" ${meta.statusOverride === 'overseas-only' ? 'selected' : ''}>Overseas only</option>
      ...
      <div class="lineup-card__table-wrap">
        <table class="lineup-card__table">
          ...
          <tbody>${seasonRows || '<tr><td colspan="4">No season details loaded yet.</td></tr>'}</tbody>
        </table>
      </div>
      ${episodeWatchHtml}
      ...
      try {
        ...
        fullDetailHost.querySelectorAll('[data-episode-watch]').forEach((checkbox) => {
          ...
          checkbox.addEventListener('change', async (event) => {
            ...
            const updatedShow = setEpisodeWatched(entry.id, season, episode, Boolean(event.currentTarget.checked));
