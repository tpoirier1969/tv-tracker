...
function cacheElements() {
  ...
  els.lineupCountText = document.getElementById('lineupCountText');
...
function mergeLoadedState(parsed) {
  ...
  if (parsed.lineupDetailExpanded && typeof parsed.lineupDetailExpanded === 'object') state.lineupDetailExpanded = parsed.lineupDetailExpanded;
...
function normalizeState() {
  ...
  state.lineupDetailExpanded = Object.fromEntries(Object.entries(state.lineupDetailExpanded || {}).filter(([id]) => state.shows.some((show) => show.id === id)));
}
...
function normalizeShow(show) {
  ...
  return {
    ...
    archived: Boolean(show?.archived ?? show?.isArchived ?? show?.is_archived ?? show?.watched?.__tvtMeta?.archived),
    archivedAt: show?.archivedAt || show?.archived_at || show?.watched?.__tvtMeta?.archivedAt || '',
    archivedSeasons: normalizeArchivedSeasonList(show?.archivedSeasons ?? show?.archived_seasons ?? show?.watched?.__tvtMeta?.archivedSeasons),
    statusOverride: show?.statusOverride || show?.status_override || show?.watched?.__tvtMeta?.statusOverride || '',
    statusNote: show?.statusNote || show?.status_note || show?.watched?.__tvtMeta?.statusNote || '',
...
function normalizeArchivedSeasonList(value) {
  let raw = value;
  ...
  return [...new Set(raw.map((item) => Number(item)).filter((item) => Number.isFinite(item) && item > 0))].sort((a, b) => a - b);
}

function getArchivedSeasons(show = null) {
  return normalizeArchivedSeasonList(show?.archivedSeasons ?? show?.archived_seasons ?? show?.watched?.__tvtMeta?.archivedSeasons);
}

function isSeasonArchived(show, season) {
  return getArchivedSeasons(show).includes(Number(season) || 0);
}

function createEpisodeWatchKey(season, episode) {
  return `ep:${Number(season) || 0}:${Number(episode) || 0}`;
}

function isEpisodeWatched(show, season, episode) {
  const key = createEpisodeWatchKey(season, episode);
  return Boolean(show?.watched?.[key]);
}

function setEpisodeWatched(showId, season, episode, checked) {
  const show = state.shows.find((item) => item.id === showId);
  if (!show) return null;
  const key = createEpisodeWatchKey(season, episode);
  const watched = { ...(show.watched || {}) };
  if (checked) watched[key] = true;
  else delete watched[key];
  show.watched = watched;
  updateShowMeta(showId, {});
  show.updatedAt = new Date().toISOString();
  return show;
}

function countWatchedEpisodes(show, episodeList = []) {
  return episodeList.reduce((total, ep) => total + (isEpisodeWatched(show, ep.season, ep.number) ? 1 : 0), 0);
}

function setSeasonArchived(showId, season, archived) {
  const show = state.shows.find((item) => item.id === showId);
  if (!show) return null;
  const seasonNumber = Number(season) || 0;
  if (!seasonNumber) return show;
  const archivedSet = new Set(getArchivedSeasons(show));
  if (archived) archivedSet.add(seasonNumber);
  else archivedSet.delete(seasonNumber);
  const archivedSeasons = [...archivedSet].sort((a, b) => a - b);
  show.archivedSeasons = archivedSeasons;
  updateShowMeta(showId, { archivedSeasons });
  show.updatedAt = new Date().toISOString();
  return show;
}

function syncSeasonArchiveFromEpisodes(showId, bundle, season) {
  const show = state.shows.find((item) => item.id === showId);
  if (!show || !bundle) return show;
  const seasonNumber = Number(season) || 0;
  if (!seasonNumber) return show;
  const seasonData = (bundle.seasons || []).find((row) => Number(row?.season) === seasonNumber);
  const episodeList = Array.isArray(seasonData?.episodeList) ? seasonData.episodeList : [];
  if (!episodeList.length) return show;
  const shouldArchive = countWatchedEpisodes(show, episodeList) >= episodeList.length;
  if (shouldArchive !== isSeasonArchived(show, seasonNumber)) {
    return setSeasonArchived(showId, seasonNumber, shouldArchive);
  }
  return show;
}
...
function persistState() {
  const payload = {
    ...
    lineupDetailExpanded: state.lineupDetailExpanded,
    sync: state.sync,
  };
...
function renderStats() {
  if (els.lineupScopeLabel) {
    const scopeBits = [state.activeUserFilter === 'all' ? 'All users' : getActiveUserLabel(), state.upcomingFilter === '21' ? '2 weeks ahead + past 7 days' : 'All shows'];
...
function getUserScopedShows() {
  const scoped = state.activeUserFilter === 'all'
    ? state.shows
    : state.shows.filter((show) => show.assignedUserIds.includes(state.activeUserFilter));

  if (state.upcomingFilter === 'archived') return scoped.filter((show) => Boolean(show.archived) || getArchivedSeasons(show).length);
  return scoped.filter((show) => !show.archived);
}

function getSeasonRowsForCurrentView(show, bundle, options = {}) {
  const seasons = Array.isArray(bundle?.seasons) ? bundle.seasons : [];
  const archivedView = options.archivedView ?? (state.upcomingFilter === 'archived');
  if (show?.archived) return archivedView ? seasons : [];
  const archivedSeasons = new Set(getArchivedSeasons(show));
  return seasons.filter((season) => {
    const seasonNumber = Number(season?.season) || 0;
    const archived = seasonNumber ? archivedSeasons.has(seasonNumber) : false;
    return archivedView ? archived : !archived;
  });
}

function hasVisibleSeasonRowsForCurrentView(show, bundle) {
  if (!bundle || !Array.isArray(bundle?.seasons) || !bundle.seasons.length) return !show?.archived;
  return getSeasonRowsForCurrentView(show, bundle).length > 0;
}

function hasUnwatchedAvailableEpisode(show, bundle) {
  const seasons = getSeasonRowsForCurrentView(show, bundle, { archivedView: false });
  if (!Array.isArray(seasons) || !seasons.length) return false;
  const today = new Date().toISOString().slice(0, 10);
  return seasons.some((season) => {
    const episodeList = Array.isArray(season?.episodeList) ? season.episodeList : [];
    return episodeList.some((episode) => {
      const airdate = String(episode?.airdate || '').trim();
      if (!airdate || airdate > today) return false;
      return !isEpisodeWatched(show, episode.season, episode.number);
    });
  });
}

function getShowMeta(show = null) {
  const meta = show?.watched?.__tvtMeta || {};
  return {
    archived: Boolean(show?.archived ?? meta.archived),
    archivedAt: show?.archivedAt || meta.archivedAt || '',
    archivedSeasons: getArchivedSeasons(show),
    statusOverride: String(show?.statusOverride || meta.statusOverride || '').trim(),
    statusNote: String(show?.statusNote || meta.statusNote || '').trim(),
  };
}

function updateShowMeta(showId, patch = {}) {
  const entry = state.shows.find((item) => item.id === showId);
  if (!entry) return;
  const meta = getShowMeta(entry);
  entry.statusOverride = patch.statusOverride !== undefined ? String(patch.statusOverride || '').trim() : meta.statusOverride;
  entry.statusNote = patch.statusNote !== undefined ? String(patch.statusNote || '').trim() : meta.statusNote;
  entry.archived = patch.archived !== undefined ? Boolean(patch.archived) : meta.archived;
  entry.archivedAt = patch.archivedAt !== undefined ? String(patch.archivedAt || '') : meta.archivedAt;
  entry.archivedSeasons = patch.archivedSeasons !== undefined ? normalizeArchivedSeasonList(patch.archivedSeasons) : meta.archivedSeasons;
  entry.watched = { ...(entry.watched || {}), __tvtMeta: {
    ...(entry.watched?.__tvtMeta || {}),
    archived: Boolean(entry.archived),
    archivedAt: entry.archivedAt || '',
    archivedSeasons: [...entry.archivedSeasons],
    statusOverride: entry.statusOverride || '',
    statusNote: entry.statusNote || '',
  } };
  entry.updatedAt = new Date().toISOString();
}

function inferProviderFamily(name = '') {
  let text = String(name || '').trim();
  if (!text) return '';
  text = text
    .replace(/with ads/ig, '')
    .replace(/premium channel/ig, '')
    .replace(/roku premium channel/ig, '')
    .replace(/apple tv channel/ig, '')
    .replace(/amazon channel/ig, '')
...
function buildStatusFromOverride(entry, bundle) {
  const override = normalizeStatusOverride(getShowMeta(entry).statusOverride);
  const note = getShowMeta(entry).statusNote;
  const withNote = (base) => note ? `${base} ${note}`.trim() : base;
  if (!override || override === 'auto') return null;
  if (override === 'scheduled' && bundle?.nextEpisode && getScheduleDisposition(bundle) === 'us') {
    return { variant: 'scheduled', text: withNote(`Next scheduled: ${formatNextEpisode(bundle.nextEpisode)}`) };
  }
  const map = {
    'in-development': { variant: 'returning', text: 'Revival or continuation is in development; no US release date is confirmed yet.' },
    'returning': { variant: 'returning', text: 'New season expected, but no US release date is announced yet.' },
    'overseas-only': { variant: 'returning', text: 'Aired overseas; US release date not confirmed.' },
    'ended': { variant: 'ended', text: 'Series ended.' },
    'canceled': { variant: 'ended', text: 'Series canceled.' },
    'likely-ended': { variant: 'ended', text: 'Likely ended — no new episode in over 2 years.' },
    'no-us-date': { variant: '', text: 'No known US release date right now.' },
  };
  const hit = map[override];
  return hit ? { variant: hit.variant, text: withNote(hit.text) } : null;
}

function isUsScheduleVerified(bundle = null) {
  if (!bundle) return false;
  const show = bundle.show || {};
  if (show.usReleaseConfirmed) return true;
  const region = String(state.settings.watchRegion || 'US').toUpperCase();
  const originCountries = Array.isArray(show.originCountries) ? show.originCountries.map((item) => String(item).toUpperCase()) : [];
  const networkCountry = String(show.networkCountry || '').toUpperCase();
  return originCountries.includes(region) || networkCountry === region;
}

function getScheduleDisposition(bundle = null) {
  if (!bundle?.nextEpisode) return 'none';
  return isUsScheduleVerified(bundle) ? 'us' : 'overseas';
}

function getVerifiedNextEpisode(bundle = null) {
  return getScheduleDisposition(bundle) === 'us' ? bundle.nextEpisode : null;
}


function getRecentUsReleaseDate(bundle = null) {
  if (!bundle || !isUsScheduleVerified(bundle)) return '';
  const candidate = String(bundle?.show?.lastAirDate || '').trim();
  if (!candidate) return '';
  const dt = new Date(`${candidate}T00:00:00`);
  if (!Number.isFinite(dt.getTime())) return '';
  const delta = daysBetween(startOfToday(), dt);
  return delta <= 0 && delta >= -7 ? candidate : '';
}

function showInTwoWeekPlusRecentWindow(show, bundle = null) {
  const resolved = bundle || getCachedBundleForShow(show);
  const verifiedNext = getVerifiedNextEpisode(resolved)?.airdate;
  if (verifiedNext) {
    const dt = new Date(`${verifiedNext}T00:00:00`);
    const delta = daysBetween(startOfToday(), dt);
    if (delta >= 0 && delta <= 14) return true;
  }
  return Boolean(getRecentUsReleaseDate(resolved));
}

function getVisibleShows() {
  const scoped = getUserScopedShows();
  if (state.upcomingFilter !== '21') return scoped;
  return scoped.filter((show) => showInTwoWeekPlusRecentWindow(show));
}
...
async function hydrateViaTmdb(entry) {
  ...
  const primaryNetworkCountry = Array.isArray(details.networks) && details.networks.length ? String(details.networks[0]?.origin_country || '') : '';
  ...
  for (const season of seasons) {
    ...
    const nextEpisodeText = nextEpisode && Number(nextEpisode.season) === Number(seasonNumber) && (originCountries.includes('US') || primaryNetworkCountry === 'US') ? formatNextEpisode(nextEpisode) : '';
    ...
    seasonRows.push({
      ...
      episodeList: Array.isArray(seasonDetails.episodes)
        ? seasonDetails.episodes.map((episode) => ({
            season: Number(episode?.season_number || seasonNumber),
            number: Number(episode?.episode_number || 0),
            title: String(episode?.name || `Episode ${episode?.episode_number || ''}`).trim(),
  ...
  return {
    ...
    show: {
      ...
      originCountries,
      networkCountry: primaryNetworkCountry,
      usReleaseConfirmed: originCountries.includes('US') || primaryNetworkCountry === 'US',
    },
    seasons: seasonRows,
    nextEpisode,
    mainChannel,
...
async function hydrateViaTvmaze(entry) {
  ...
  const episodesBySeason = groupEpisodesBySeason(episodesResp || []);
  const seasonRows = (seasonsResp || []).slice().sort((a, b) => (a.number || 0) - (b.number || 0)).map((season, index) => ({
    ...
    platform: '',
    nextEpisode: nextEpisode && Number(nextEpisode.season) === Number(season.number) && ((showResp?.network?.country?.code || showResp?.webChannel?.country?.code || '') === 'US') ? formatNextEpisode(nextEpisode) : '',
    episodeList: episodesBySeason[Number(season.number) || 0] || [],
  }));
  return {
    id: entry.id,
    source: 'tvmaze',
    ...
    show: { ...showResp, lastAirDate: showResp.ended || '', inProduction: /running|to be determined|in development/i.test(String(showResp.status || '')), originCountries: [showResp?.network?.country?.code || showResp?.webChannel?.country?.code || ''].filter(Boolean), networkCountry: showResp?.network?.country?.code || showResp?.webChannel?.country?.code || '', usReleaseConfirmed: (showResp?.network?.country?.code || showResp?.webChannel?.country?.code || '') === 'US' },
    seasons: seasonRows,
    nextEpisode,
...
function groupEpisodesBySeason(episodes = []) {
  return (episodes || []).reduce((acc, episode) => {
    ...
    if (!Number.isFinite(season) || !Number.isFinite(number)) return acc;
    acc[season] = acc[season] || [];
    acc[season].push({
      season,
      number,
      title: String(episode?.name || `Episode ${number}`).trim() || `Episode ${number}`,
...
function inferLineupStatus(entry, bundle) {
  const overrideStatus = buildStatusFromOverride(entry, bundle);
  if (overrideStatus) return overrideStatus;

  const scheduleDisposition = getScheduleDisposition(bundle);
  if (scheduleDisposition === 'us') {
    return {
      variant: 'scheduled',
  ...
  const recentUsRelease = getRecentUsReleaseDate(bundle);
  if (recentUsRelease) {
    return {
      variant: 'scheduled',
      text: `Dropped in past 7 days: ${formatDateLong(recentUsRelease)}`,
    };
  }
  ...
  if (lastAirDate) {
    ...
    if (ageDays !== null && ageDays > 730) {
      return { variant: 'ended', text: attachNote('Likely ended — no new episode in over 2 years.') };
    }
  }

  return bundle
    ? { variant: '', text: attachNote('No known US release date right now.') }
...
function summarizeLineupStatus(status = {}) {
  ...
  if (text.startsWith('Dropped in past 7 days:')) return text.replace('Dropped in past 7 days:', 'Dropped').trim();
  ...
  if (text.includes('Returning, but no US release date')) return 'US date pending';
  if (text.includes('Aired overseas; US release date not confirmed')) return 'Overseas only';
  if (text.includes('New season expected')) return 'Returning';
  if (text.includes('Loading details')) return 'Loading';
  if (text.includes('No known US release date')) return 'No US date';
...
function setLineupExpanded(showId, expanded) {
  ...
  if (!expanded) setLineupDetailExpanded(showId, false, { silent: true });
  persistState();
  renderLineup();
}

function setLineupDetailExpanded(showId, expanded, { silent = false } = {}) {
  state.lineupDetailExpanded = state.lineupDetailExpanded || {};
  state.lineupDetailExpanded[showId] = Boolean(expanded);
  if (expanded) {
    state.lineupExpanded = state.lineupExpanded || {};
    state.lineupExpanded[showId] = true;
  }
  persistState();
  if (!silent) renderLineup();
}
...
function buildInlineDetailHtml(entry, bundle) {
  if (!bundle) return '<div class="lineup-card__full"><p class="lineup-card__full-empty">Loading full details…</p></div>';

  const assignedUsers = getAssignedUsers(entry);
  const assignedText = assignedUsers.length ? assignedUsers.map((user) => user.name).join(', ') : (state.users.length ? 'Unassigned' : 'Shared lineup');
  const summary = stripHtml(bundle.show?.summary || 'No summary available.');
  const poster = bundle.show?.image?.medium || '';
  const safeNext = getVerifiedNextEpisode(bundle);
  const nextText = safeNext ? formatNextEpisode(safeNext) : inferLineupStatus(entry, bundle).text;
  const meta = getShowMeta(entry);
  const archivedView = state.upcomingFilter === 'archived';
  const visibleSeasons = getSeasonRowsForCurrentView(entry, bundle, { archivedView });
  const seasonRows = visibleSeasons.slice(0, 8).map((season) => {
    const seasonNext = safeNext && Number(safeNext.season) === Number(season.season) ? formatNextEpisode(safeNext) : '—';
    return `<tr><td>${escapeHtml(String(season.season ?? '—'))}</td><td>${escapeHtml(String(season.episodes || '—'))}</td><td>${escapeHtml(season.releaseDate || 'TBA')}</td><td>${escapeHtml(seasonNext)}</td></tr>`;
  }).join('');

  const episodeSeasonBlocks = visibleSeasons.map((season) => {
    const episodeList = Array.isArray(season.episodeList) ? season.episodeList : [];
    if (!episodeList.length) return '';
    const seasonNumber = Number(season.season) || 0;
    const watchedCount = countWatchedEpisodes(entry, episodeList);
    const seasonArchived = isSeasonArchived(entry, seasonNumber);
    const seasonAction = archivedView && !entry.archived
      ? `<button class="ghost-btn small episode-watch-season__action" type="button" data-season-archive-toggle data-season="${escapeAttr(String(seasonNumber))}">Restore season</button>`
      : '';
    const episodeItems = episodeList.map((episode) => {
      const checked = isEpisodeWatched(entry, episode.season, episode.number) ? 'checked' : '';
      const code = `S${episode.season}E${episode.number}`;
      const title = episode.title || `Episode ${episode.number}`;
      const air = episode.airdate ? `<span class="episode-watch-item__air">${escapeHtml(episode.airdate)}</span>` : '';
      return `<label class="episode-watch-item"><input type="checkbox" data-episode-watch data-season="${escapeAttr(String(episode.season))}" data-episode="${escapeAttr(String(episode.number))}" ${checked}><span class="episode-watch-item__code">${escapeHtml(code)}</span><span class="episode-watch-item__title-wrap"><span class="episode-watch-item__title">${escapeHtml(title)}</span>${air}</span></label>`;
    }).join('');
    return `<details class="episode-watch-season" ${archivedView ? 'open' : ''} data-season-block="${escapeAttr(String(seasonNumber))}"><summary><span>Season ${escapeHtml(String(season.season ?? '—'))}</span><span class="episode-watch-season__count">${watchedCount}/${episodeList.length} watched</span>${seasonAction}</summary><div class="episode-watch-list">${episodeItems}</div>${seasonArchived && !entry.archived ? '<p class="episode-watch-panel__hint">This season is archived because every tracked episode is checked watched.</p>' : ''}</details>`;
  }).filter(Boolean).join('');

  const episodeWatchHtml = episodeSeasonBlocks
    ? `<section class="episode-watch-panel"><h4>${archivedView ? 'Archived season tracker' : 'Episode watch tracker'}</h4><p class="episode-watch-panel__hint">${archivedView ? 'Completed seasons live here. Restore one if you want it back in the active lineup.' : 'Check off episodes you have watched. When a whole season is checked off, it moves to Archived automatically.'}</p>${episodeSeasonBlocks}</section>`
    : (!entry.archived ? `<section class="episode-watch-panel"><h4>${archivedView ? 'Archived season tracker' : 'Episode watch tracker'}</h4><p class="episode-watch-panel__hint">${archivedView ? 'No archived seasons for this series yet.' : 'Episode-level data is not loaded for this series yet.'}</p></section>` : '');

  return `<div class="lineup-card__full">
      <div class="lineup-card__hero">
        ${poster ? `<img class="lineup-card__poster" src="${poster}" alt="${escapeAttr(entry.name)} poster" />` : '<div class="lineup-card__poster placeholder">No art</div>'}
        <div>
          <h4>${escapeHtml(entry.name)}</h4>
          <p><strong>Users:</strong> ${escapeHtml(assignedText)}</p>
          <p><strong>Status:</strong> ${escapeHtml(nextText)}</p>
          <p>${escapeHtml(summary)}</p>
        </div>
      </div>
      <div class="lineup-card__status-editor">
        <label>
          <span>Status override</span>
          <select class="lineup-card__status-override">
            <option value="auto" ${!meta.statusOverride ? 'selected' : ''}>Auto</option>
            <option value="scheduled" ${meta.statusOverride === 'scheduled' ? 'selected' : ''}>Scheduled</option>
            <option value="dropped" ${meta.statusOverride === 'dropped' ? 'selected' : ''}>Dropped recently</option>
            <option value="returning" ${meta.statusOverride === 'returning' ? 'selected' : ''}>Returning / renewed</option>
            <option value="in-development" ${meta.statusOverride === 'in-development' ? 'selected' : ''}>Revival / continuation in development</option>
            <option value="no-us-date" ${meta.statusOverride === 'no-us-date' ? 'selected' : ''}>No US date</option>
            <option value="ended" ${meta.statusOverride === 'ended' ? 'selected' : ''}>Ended</option>
            <option value="canceled" ${meta.statusOverride === 'canceled' ? 'selected' : ''}>Canceled</option>
            <option value="likely-ended" ${meta.statusOverride === 'likely-ended' ? 'selected' : ''}>Likely ended</option>
          </select>
        </label>
        <label>
          <span>Status note</span>
          <input class="lineup-card__status-note" type="text" value="${escapeAttr(meta.statusNote)}" placeholder="Optional note for edge cases like revivals" />
        </label>
        <button class="ghost-btn small lineup-card__save-status" type="button">Save status</button>
      </div>
      <div class="lineup-card__table-wrap">
        <table class="lineup-card__table">
          <thead><tr><th>Season</th><th>Episodes</th><th>Original release</th><th>US next</th></tr></thead>
          <tbody>${seasonRows || `<tr><td colspan="4">${archivedView ? 'No archived seasons for this series yet.' : 'No active seasons are showing here right now.'}</td></tr>`}</tbody>
        </table>
      </div>
      ${episodeWatchHtml}
      <div class="lineup-card__full-footer">
        <button class="ghost-btn small lineup-card__hide-details" type="button">Hide details</button>
      </div>
    </div>`;
}
...
function renderLineup() {
  try {
    ...
    for (const show of scopedShows) {
      ...
      if (state.upcomingFilter === '21' && bundle && !showInTwoWeekPlusRecentWindow(show, bundle)) continue;
      if (state.upcomingFilter === '21' && !bundle) {
        visibleShows.push({ entry: show, bundle: null });
        continue;
      }
      if (bundle && !hasVisibleSeasonRowsForCurrentView(show, bundle)) continue;
      visibleShows.push({ entry: show, bundle });
    }
    queueLineupHydration(scopedShows);
    if (els.lineupCountText) els.lineupCountText.textContent = `${visibleShows.length} visible`;
    const title = document.getElementById('lineupTitleText');
    if (title) {
      if (state.upcomingFilter === 'archived') title.textContent = 'Archived shows';
      else title.textContent = 'Lineup';
    }
    if (!visibleShows.length) {
      els.lineupGrid.className = 'lineup-grid empty-state-box';
      const message = state.upcomingFilter === 'archived'
        ? 'No archived shows yet.'
        : (state.activeUserFilter === 'all' ? 'No shows match this lineup filter yet.' : `No shows match this filter for ${escapeHtml(getActiveUserLabel())} yet.`);
      els.lineupGrid.innerHTML = `<p>${message}</p>`;
      return;
    }
    ...
    for (const { entry, bundle } of visibleShows) {
      try {
        ...
        const pendingBadge = card.querySelector('.lineup-card__pending-indicator');
        const titleRowBtn = card.querySelector('.lineup-card__title-row');
        const openBtn = card.querySelector('.lineup-card__open');
        const archiveBtn = card.querySelector('.lineup-card__archive');
        const fullDetailHost = card.querySelector('.lineup-card__full-detail');
        ...
        const hasPendingEpisode = hasUnwatchedAvailableEpisode(entry, bundle);
        ...
        if (pendingBadge) {
          pendingBadge.hidden = !hasPendingEpisode;
          pendingBadge.setAttribute('aria-hidden', String(!hasPendingEpisode));
          pendingBadge.title = hasPendingEpisode ? 'Unwatched released episode waiting' : '';
        }
        titleRowBtn.setAttribute('aria-expanded', String(expanded));
        titleRowBtn.setAttribute('aria-label', expanded ? 'Collapse listing' : 'Expand listing');
        card.classList.toggle('lineup-card--expanded', expanded);
        card.classList.toggle('lineup-card--collapsed', !expanded);
        card.classList.toggle('lineup-card--full-open', detailExpanded);
        if (status.variant) card.classList.add(`lineup-card--${status.variant}`);

        openBtn.textContent = detailExpanded ? 'Hide details' : 'Details';
        archiveBtn.textContent = entry.archived ? 'Restore' : 'Archive';
        archiveBtn.classList.toggle('ghost-btn', !entry.archived);
        archiveBtn.classList.toggle('primary-btn', Boolean(entry.archived));
        archiveBtn.classList.add('small');

        if (detailExpanded) fullDetailHost.innerHTML = buildInlineDetailHtml(entry, bundle);
        else fullDetailHost.innerHTML = '';

        const openInlineDetails = async () => {
          const nextState = !Boolean(state.lineupDetailExpanded?.[entry.id]);
          if (nextState && !getCachedBundleForShow(entry)) {
            try {
              await hydrateShow(entry.id);
            } catch (err) {
              console.error(err);
            }
          }
          setLineupDetailExpanded(entry.id, nextState);
        };
        openBtn.addEventListener('click', openInlineDetails);
        card.querySelector('.lineup-card__assign').addEventListener('click', () => openAssignModal(entry.id));
        card.querySelector('.lineup-card__delete').addEventListener('click', (event) => { event.stopPropagation(); removeShow(entry.id); });
        titleRowBtn.addEventListener('click', () => setLineupExpanded(entry.id, !expanded));
        titleRowBtn.addEventListener('keydown', (event) => {
          if (event.key === 'Enter' || event.key === ' ') {
            event.preventDefault();
            setLineupExpanded(entry.id, !expanded);
          }
        });
        archiveBtn.addEventListener('click', () => toggleArchiveShow(entry.id));
        fullDetailHost.querySelector('.lineup-card__hide-details')?.addEventListener('click', () => setLineupDetailExpanded(entry.id, false));
        fullDetailHost.querySelector('.lineup-card__save-status')?.addEventListener('click', async () => {
          const overrideValue = fullDetailHost.querySelector('.lineup-card__status-override')?.value || '';
          const noteValue = fullDetailHost.querySelector('.lineup-card__status-note')?.value || '';
          updateShowMeta(entry.id, { statusOverride: overrideValue === 'auto' ? '' : overrideValue, statusNote: noteValue });
          persistState();
          if (hasSupabaseConfig()) {
            try {
              await upsertRemoteShows([state.shows.find((item) => item.id === entry.id)]);
              markSyncSuccess();
            } catch (err) {
              console.error(err);
              markSyncError(err);
          ...
          render();
          toast('Status override saved.');
        });
        fullDetailHost.querySelectorAll('.episode-watch-panel, .episode-watch-season, .episode-watch-list, .episode-watch-item').forEach((node) => {
          node.addEventListener('click', (event) => event.stopPropagation());
        });
        fullDetailHost.querySelectorAll('[data-season-archive-toggle]').forEach((button) => {
          button.addEventListener('click', async (event) => {
            event.stopPropagation();
            const seasonNumber = Number(event.currentTarget.dataset.season || 0);
            const updatedShow = setSeasonArchived(entry.id, seasonNumber, false);
            if (!updatedShow) return;
            persistState();
            render();
            if (hasSupabaseConfig()) {
              try {
                await upsertRemoteShows([updatedShow]);
                markSyncSuccess();
              } catch (err) {
                console.error(err);
                markSyncError(err);
            ...
            toast(`Season ${seasonNumber} restored to the active lineup.`);
          });
        });
        fullDetailHost.querySelectorAll('[data-episode-watch]').forEach((checkbox) => {
          checkbox.addEventListener('click', (event) => event.stopPropagation());
          checkbox.addEventListener('change', async (event) => {
            event.stopPropagation();
            const season = Number(event.currentTarget.dataset.season || 0);
            const episode = Number(event.currentTarget.dataset.episode || 0);
            let updatedShow = setEpisodeWatched(entry.id, season, episode, Boolean(event.currentTarget.checked));
            if (!updatedShow) return;
            const wasArchived = isSeasonArchived(updatedShow, season);
            updatedShow = syncSeasonArchiveFromEpisodes(entry.id, bundle, season) || updatedShow;
            persistState();
            const isArchivedNow = isSeasonArchived(updatedShow, season);
            if (wasArchived !== isArchivedNow) {
              render();
              if (isArchivedNow) toast(`Season ${season} moved to Archived.`);
            } else {
              refreshEpisodeWatchPanel(updatedShow, bundle, fullDetailHost);
            }
            if (hasSupabaseConfig()) {
              try {
                await upsertRemoteShows([updatedShow]);
...
function toggleArchiveShow(showId) {
  const show = state.shows.find((item) => item.id === showId);
  if (!show) return;
  const archiving = !show.archived;
  show.archived = archiving;
  show.archivedAt = archiving ? new Date().toISOString() : '';
  show.updatedAt = new Date().toISOString();
  if (archiving) {
    state.lineupExpanded[showId] = false;
    state.lineupDetailExpanded[showId] = false;
    if (state.selectedId === showId) state.selectedId = null;
  }
  persistState();
  render();
  const actionText = archiving ? 'archived' : 'restored';
  if (hasSupabaseConfig()) {
    upsertRemoteShows([show]).then(() => markSyncSuccess()).catch((err) => {
      console.error(err);
      toast(`Show ${actionText} locally, but cloud sync failed: ${friendlySyncError(err)}`);
    });
  }
  toast(`${show.name} ${actionText}.`);
}
...
function refreshEpisodeWatchPanel(show, bundle, fullDetailHost) {
  if (!show || !bundle || !fullDetailHost) return;
  const tracker = fullDetailHost.querySelector('.episode-watch-panel');
  if (!tracker) return;
  tracker.querySelectorAll('.episode-watch-season').forEach((seasonNode) => {
    const firstCheckbox = seasonNode.querySelector('[data-episode-watch]');
    if (!firstCheckbox) return;
    const seasonNumber = Number(firstCheckbox.dataset.season || 0);
    const seasonData = (bundle.seasons || []).find((season) => Number(season.season) === seasonNumber);
    const episodeList = Array.isArray(seasonData?.episodeList) ? seasonData.episodeList : [];
    const countEl = seasonNode.querySelector('.episode-watch-season__count');
    if (countEl) countEl.textContent = `${countWatchedEpisodes(show, episodeList)}/${episodeList.length} watched`;
  });
}
...
function buildWatchedForRemote(show) {
  const watched = { ...(show.watched || {}) };
  watched.__tvtMeta = {
    archived: Boolean(show.archived),
    archivedAt: show.archivedAt || '',
    archivedSeasons: getArchivedSeasons(show),
    statusOverride: show.statusOverride || '',
...
function serializeShowForRemote(show) {
  return {
    ...
    name: show.name,
    premiered: show.premiered || '',
    watched: buildWatchedForRemote(show),
